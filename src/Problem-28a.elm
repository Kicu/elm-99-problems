...
test =
    List.length
        <| List.filter ((==) False)
            [ List.map List.length (sortByListLengths [ [], [ 1 ], [1,2], (List.range 1 3), (List.range 1 4), (List.range 1 5) ])
                == [ 0, 1, 2, 3, 4, 5 ]
            , List.map List.length (sortByListLengths [ [] ])
                == [ 0 ]
            , List.map List.length (sortByListLengths [ [], [ 1 ], (List.range 1 100000), (List.range 1 4), (List.range 1 3), [1,2] ])
                == [ 0, 1, 2, 3, 4, 100000 ]
